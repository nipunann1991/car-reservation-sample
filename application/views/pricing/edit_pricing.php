	...
	<div class="container">
		...
		<form  id="myform" class="col-md-5 box_form" novalidate="novalidate"> 
			...
			<div class="row">
		  	...
		  	<button type="submit" class="btn btn-primary"><i class="fa fa-plus-circle" aria-hidden="true"></i> Edit Pricing </button> 
...
<script>
	$(document).ready(function () {  
    	...
    	setTimeout(function(){
	    	...
	    	$.ajax({
				url: '<?php echo base_url(); ?>index.php/pricing_controller/get_single_pricing_data',
        		type: 'POST', 
        		data: { pricing_id: getQueryVariable("pricing_id")},
			})
			.done(function(data) {
	 			...
	 			if (output.status == 200) {
					...
					$('#price_per_day').val(output.data.price_per_day);
					$('#price_per_hour').val(output.data.price_per_hour);
					$('#price_per_km').val(output.data.price_per_km);
